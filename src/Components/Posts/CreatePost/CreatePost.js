...
import {query, gen_emotion} from '../../../utils/requestEmotion';
...
export class CreatePost extends Component {
    ...
    componentDidMount() {
        ...
        if (path === "/edit/:id") {
            ...
            Axios.get("/posts/" + id).then(data => {
                ...
                this.setState({
                    isloading: false,
                    Post: { ...this.state.Post, id: post._id, title: post.title, content: post.content, imagePath: post.imagePath, emotion: post.emotion }
                });
            })
                .catch(e => {
                    this.setState({
    ...
    mySubmitHandler(e) {
        ...
        e.preventDefault()
        let formData;
        query({"inputs": this.state.Post.content}).then((response) => {
            console.log(gen_emotion(response))
            this.setState({
                Post: { ...this.state.Post, emotion: gen_emotion(response)}
            });
            if (typeof (this.state.Post.imagePath) === "object") {
                ...
                formData.append('emotion', this.state.Post.emotion);
            }
            else {
                formData = {
                    ...
                    'emotion': this.state.Post.emotion
                }
            }
            console.log("good")
            console.log(this.state.Post.emotion)
            ...
            this.setState({
                Post: { ...this.state.Post, title: '', content: '', imagePath: '', emotion: '' }
            });
        }
        );
